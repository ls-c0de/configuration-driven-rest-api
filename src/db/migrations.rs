...
pub async fn start_migrations(db: Pool<Sqlite>) {
    ...
    for model in config.model {
        create_tables(db.clone(), model.tablename, model.fields).await.unwrap(); // Clone needed?
    }
}
...
pub async fn create_tables(db: Pool<Sqlite>, table_name: String, fields: Vec<Fields<String>>) -> Result<(), sqlx::Error> {
